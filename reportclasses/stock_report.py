...
class GenerateStockReportExcell:
    ...
    def generate(self):
        ...
        worksheet.write('B5', 'Prodcut/Item')
        worksheet.write('C5', 'Purchases')
        worksheet.write('D5', 'Sales')
        worksheet.write('E5', 'Free Issues')
        worksheet.write('F5', 'Market Returns')
        ...
        for row, item in enumerate(self.item_details, start=1):
            worksheet.write(row+5, 0,  self.main_details['date'])
            worksheet.write(row+5, 1, item['product_name'])
            worksheet.write(row+5, 2, item['purchase'])
            worksheet.write(row+5, 3, item['sales'])
            worksheet.write(row+5, 4, item['free_issues'])
            worksheet.write(row+5, 5, item['market_returns'])
