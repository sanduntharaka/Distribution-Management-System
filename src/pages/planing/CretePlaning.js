...
const CretePlaning = () => {
    ...
    const fetchPsas = (item) => {
        axiosInstance
            .get(`/psa/get/srep/${item.salesref}`, {
                headers: {
                    Authorization:
    ...
    const fetchAprrovePlaning = () => {
        console.log('called')
        axiosInstance
            .get(`/planing/approve/to/`, {
                headers: {
                    Authorization:
                        'JWT ' + JSON.parse(sessionStorage.getItem('userInfo')).access,
                },
    ...
    const [planing, setPlaning] = useState(NaN)
    const handleSalesRef = (item) => {
        console.log(item)
        axiosInstance.get(`/planing/get/${item.salesref}/${item.date}`, {
            headers: {
                Authorization:
                    'JWT ' + JSON.parse(sessionStorage.getItem('userInfo')).access,
            },
        ...
        <div className="page">
            ...
            <div className="page__pcont">
                ...
                <div className="page__pcont__row" style={{ background: "#cccccc" }}>
                    ...
                    <p style={{ margin: 5 }}>You have {planings.length} Planing to Approve </p>
                    ...
                    <table border="1">
                        ...
                        <tbody>

                            {
                                planings.map((item, i) => (
                                    <tr key={i}>
                                        <td>{item.salesref_name}</td>
                                        <td>{item.date}</td>
                                        <td>{item.psas_area_names.map((psa, j) =>
                                            <p key={j}>{psa},</p>
                                        )}</td>
                                        <td><button className="addBtn" onClick={() => handleSalesRef(item)}>Show</button></td>
                        ...
                        {/* <div className="form__row">
                            <div className="form__row__col">
                                ...
                                <div className="form__row__col__input">
                                    ...
                                    <input type="date" placeholder="Select Date" onChange={(e) =>
                                        setDay(e.target.value)
                                    } />
                                </div>
                            </div>
