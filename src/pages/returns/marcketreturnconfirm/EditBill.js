...
const tableIcons = {
  ...
  Clear: forwardRef((props, ref) => (
    <Clear {...props} ref={ref} style={{ color: 'green' }} />
  )),
...
const EditBill = (props) => {
  ...
  useEffect(() => {
    axiosInstance
      .get(`/salesref/return/get/items/${props.invoice.id}`, {
        headers: {
          Authorization:
  ...
  const handleEdit = (newData, oldData, resolve) => {
    ...
    setSuccess(false);
    axiosInstance
      .put(`/salesref/return/update/item/${newData.id}`, newData, {
        headers: {
          Authorization:
            'JWT ' + JSON.parse(sessionStorage.getItem('userInfo')).access,
        },
  ...
  const handleDelete = (oldData, resolve) => {
    ...
    setSuccess(false);
    axiosInstance
      .delete(`/salesref/return/delete/item/${oldData.id}`, oldData, {
        headers: {
          Authorization:
            'JWT ' + JSON.parse(sessionStorage.getItem('userInfo')).access,
        },
